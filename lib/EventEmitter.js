...
class EventEmitter {
    ...
    emit(eventName) {
        ...
        if (this.paused) {
            this.emitQueue = _immunity2.default.appendToArray(this.emitQueue, { async: false, eventName: eventName, params: args });
    ...
    emitAsync(eventName) {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (_this.paused) {
                _this.emitQueue = _immunity2.default.appendToArray(_this.emitQueue, { async: true, eventName: eventName, params: args });
    ...
    resume() {
        ...
        for (const item of this.emitQueue) {
            if (item.async) {
                this.emitAsync.apply(this, [item.eventName].concat(_toConsumableArray(item.params)));
            } else {
                this.emit.apply(this, [item.eventName].concat(_toConsumableArray(item.params)));
            }
        }
